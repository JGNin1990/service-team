...
import {
    fetch_loged_team_member,
    fetch_service_report_list,
} from '../../src/redux/Team_memberReducer';
import {
...
const RequestScreen = clientdata => {
    ...
    const filtered_clientdata = clientdata.clientdata.filter(
        value =>
            value.service_status == 3 &&
            clientdata.user.service_team_admin_id ==
                value.approved_by_service_admin,
    );
            ...
            <ScrollView style={{flex: 1, backgroundColor: 'white'}}>
                <View
                    ...
                    {item.photo ? (
                        <Image
                            source={{
                                uri: `${PHOTO_URL}` + '/client/' + item.photo,
                            }}
                            style={{
                                width: 40,
                                height: 40,
                    ...
                    <TouchableOpacity
                        ...
                        <Text style={{color: '#654EA3', fontSize: 11}}>
                            Report
                        </Text>
                    </TouchableOpacity>
                </View>
    ...
    const searchfilter = text => {
        let searchdata = masterData.filter(item => {
            return (
                ...
                item.last_name.toUpperCase().includes(text.toUpperCase())
            );
        });
        setSearch(text);
...
const RejectScreen = rejected_client => {
    const filtered_data = rejected_client.reject_data.filter(
        val =>
            val.service_team_admin_id ==
                rejected_client.user.service_team_admin_id &&
            val.status == 2 &&
            val.report_type == 0,
    );
    const navigation = useNavigation();
...
function CompletedScreen(complete_clientdata) {
    ...
    const navigation = useNavigation();
    const filtered_data = complete_clientdata.complete_clientdata.filter(
        value =>
            value.service_status == 4 &&
            value.approved_by_service_admin ==
                complete_clientdata.user.service_team_admin_id,
    );
    const [filteredData, setfilteredData] = useState(filtered_data);
    const [masterData, setmasterData] = useState(filtered_data);
    const [search, setSearch] = useState('');
            ...
            <ScrollView style={{flex: 1, backgroundColor: 'white'}}>
                <View
                    ...
                    <TouchableOpacity
                        ...
                        onPress={() =>
                            navigation.navigate('Servicehistory', item)
                        }>
                        <Text style={{color: '#654EA3', fontSize: 11}}>
                            View
                        </Text>
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <Tab.Navigator
                ...
                <Tab.Screen name="Request List">
                    {() => (
                        <RequestScreen
                            clientdata={props.all_client_list}
                            user={props.loged_user}
                        />
                    )}
                </Tab.Screen>
                <Tab.Screen name="Reject List">
                    {() => (
                        <RejectScreen
                            user={props.loged_user}
                            reject_data={all_service_list}
                        />
                    )}
                </Tab.Screen>
                <Tab.Screen name="Complete List">
                    {() => (
                        <CompletedScreen
                            complete_clientdata={props.all_client_list}
                            user={props.loged_user}
                        />
                    )}
                </Tab.Screen>
